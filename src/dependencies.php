...
$container['sentinel'] = function () {
    $bootstrapper = new \Cartalyst\Sentinel\Native\SentinelBootstrapper(
        ROOT_PATH . '/config/sentinel.php'
    );
    $sentinel = new \Cartalyst\Sentinel\Native\Facades\Sentinel($bootstrapper);
...
$container['user'] = function ($container) {
    $userMailer = new App\Models\UserMailer(
        $container->mailer,
        ROOT_PATH . '/templates/emails'
    );
    $userMailer->setFrom($container->settings['app']['mailer_from']);
    $userMailer->setSiteName($container->settings['app']['metas']['title']);

    $user = new App\Models\User($container->sentinel, $userMailer);
    $user->allowRememberMe($container->settings['security']['enable_remember_me']);
    $user->setSendActivationEmail($container->settings['security']['new_account_requires_email_activation']);
